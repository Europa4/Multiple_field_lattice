...
for n in np.arange(n_files):
    #data location
    file_name = 'Delta_0-1500/phi_' + str(n)
    ...
    chi_data = phi_data[:, prot.Ntot:]
    phi_data = phi_data[:, :prot.Ntot]
    ...
    for i in np.arange(int(number_of_iterations)):
        ...
        Obs = observable(chi_data[i, :])
    #This calculates the expression fully for one initiatlisation
    expectation_observable[n, :, :] = np.mean(numerator_phi, axis = 0)/denominator
    
    #error analysis for this file
    for i in np.arange(prot.Nt):
        for j in np.arange(prot.Nt):
            file_error[n, i, j] = jackknife(np.real(numerator_phi[:, i, j]/denominator), jackknife_block_length) + prot.j*jackknife(np.imag(numerator_phi[:, i, j]/denominator), jackknife_block_length)
