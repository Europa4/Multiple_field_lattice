...
def observable(phi_data, header):
    ...
    quantum_quantum_vector = np.concatenate(([2*phi_data[0]],quantum_quantum_vector))
    quantum_quantum_vector = np.reshape(quantum_quantum_vector, (q_valid_range, 1))
    quantum_quantum_vector_transpose = np.transpose(quantum_quantum_vector)
    quantum_quantum_correlator = np.matmul(quantum_quantum_vector, quantum_quantum_vector_transpose)
    return quantum_quantum_correlator
...
for i in np.arange(prot.Nt - 2):
    ...
    plt.title('Quantum quantum i = ' + str(x_range[i]))
    plt.xlabel('j')
    plt.ylabel(r'$\langle \phi^{q}_j \phi^{q}_i \rangle$')
    plt.legend()    
    plt.savefig(location + 'q_q_' + str(i))
    plt.plot()
