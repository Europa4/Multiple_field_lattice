...
def observable(phi_data, header, p):
    ...
    i_phi = np.reshape(phi, (prot.Nx, prot.Nrp))
    i_phi = np.append(header[])
    phi_k = FT.ft(i_phi, p, prot.deltaX)
    phi_nk = FT.ft(i_phi, -1*p, prot.deltaX)
    phi_k = np.reshape(phi_k, (1, prot.Nrp))
    phi_nk = np.reshape(phi_nk, (prot.Nrp, 1))
