...
location = "/run/media/ppxsw1/78fe3857-1897-4617-a65e-83c9aa61be27/2_field_large_links_test_low_mass/"
...
for i in np.arange(prot.Nt):
    plt.figure(i)
    plt.errorbar(x_range, np.real(final_expectation_phi[:, i]).flatten(), yerr = np.real(final_error_phi[:, i])/5, label = "real", fmt = '', linestyle = 'None', marker = '.', capsize = 2)
    plt.errorbar(x_range, np.imag(final_expectation_phi[:, i]).flatten(), yerr = np.imag(final_error_phi[:, i])/5, label = "imag", fmt = '', linestyle = 'None', marker = '.', capsize = 2)
